...
$P=C'(X)$ innbærer at kvantumer gitt ved
...
Konsumentoverskudd
\begin{equation*}
KO=\frac{50(500-400)}{2}=2500
\end{equation*}
Produsentoverskudd
\begin{equation*}
PO=\frac{50(366-100)}{2}+(50)\cdot(400-366)=10000
\end{equation*}
Effektivitetstap (dødvektstap)
\begin{equation*}
ST=\frac{(66.7-50)\cdot(400-366.7)}{2}=835
\end{equation*}
...
mp102 <- paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/drawio/10.2_mp2.png')
knitr::include_graphics(mp102)
```
...
```{r, echo=FALSE, out.width="50%", fig.cap=""}
markedsformer <- paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/drawio/markedsformer.png')
knitr::include_graphics(markedsformer)
```

- I dette kurset har vi spesielt ønske om å studere markedsformen *monopolistisk konkurranse* nærmere.
...
- Lang sikt
	...
	- Det fører til redusert etterspørsel etter det produktet som vi ser på, helt inntil fortjenesten er redusert til tilnærmet lik null.
...
1. **Ufullkommen konkurranse og anvendelse av markedsmakt** (sett på tidligere)
1. **Eksterne virkninger** (ser på nå)
1. Kollektive goder
1. Formyndergoder
...
```{r, echo=FALSE, out.width="45%", fig.cap=""}
knitr::include_graphics(c("drawio/eks_ill.png"))
```
...
#### Negative eksternaliteter under fullkommen konkurranse 

Anta det er negative eksternaliteter ved produksjonen. Det betyr at den samfunnsøkonomiske grensekostnaden består av bedriftenes grensekostnader pluss marginal ekstern kostnad MEK, dvs. $GK + MEK$. 
...
Den samfunnsøkonomisk effektive mengden er $x_{SO}$ enheter kjennetegnet ved at marginal betalingsvilje (gjenspeilet i etterspørselskurven) er lik den samfunnsøkonomiske grensekostnaden $GK + MEK$.

---

#### Mulige løsninger (offentlige myndigheter)
...
Ved å ilegge en avgif $t= MEK$ per produsert enhet vil markeds­tilbudet skifte opp tilsvarende MEK og det etableres en ny markedslikevekt med markedspris $p_{KP}$ og likevektsmengde $x_{SO}$. Dermed vil avgiften i dette tilfellet fjerne et effektivitetstap og altså bidra til en samfunnsøkonomisk mer effektiv ressursbruk.
...
Alternativt kan samme produksjonsnivå $x$ oppnås ved at myndighetene innfører en kvote som regulerer produksjonen slik at den ikke blir høyere enn det som er samfunnsøkonomisk optimalt.
