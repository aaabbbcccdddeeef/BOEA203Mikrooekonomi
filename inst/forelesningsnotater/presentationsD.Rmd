...
pathn <- '/inst/forelesningsnotater/presentationsD.Rmd'
xaringan::inf_mr(paste0(rprojroot::find_rstudio_root_file(),pathn))
system(paste0('brave ',rprojroot::find_rstudio_root_file(),pathn))
servr::daemon_stop()
rmarkdown::render(paste0(rprojroot::find_rstudio_root_file(),pathn))
```
...
```{r, echo=FALSE}
library(dplyr)
tabell_prodf <- data.frame(N=seq(1,10),K=10,x=10,GP='-',GjP='1')
tipf_navn <- c("Arbeidskraft (N)","Kapital (K)","Produksjon", "Grenseprodukt.", "Gjennomsnittsprod.")
kableExtra::kbl(tabell_prodf, col.names=tipf_navn)
```
...
```{r, echo=FALSE, out.width="40%", fig.cap=""}
pkk <- paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/drawio/prodkl.png')
...
- Isokvantens form bygger på følgende prinsipp: jo mer bedriften har av en innsatsfaktor, jo mer kan den bytte for en ekstra enhet av den andre faktoren, gitt at produksjonsmengden skal være den samme.
...
.pull-left[
Linear produksjonsteknolog (perfekte substitutter)
...
.pull-right[
Lientof produksjonsteknologi (fast produksjonsforhold)
...
```{r, echo=FALSE, out.width="40%", fig.cap=""}
ksa <- paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/drawio/okskala.png')
knitr::include_graphics(ksa)
```
...
```{r, echo=FALSE, out.width="40%", fig.cap=""}
ksa <- paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/drawio/avskala.png')
knitr::include_graphics(ksa)
```
